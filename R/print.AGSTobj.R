...
cat(paste("\n cer: ",round(x[[3]]$al,digits=3),"\t SF: ",x[[3]]$SF,"\t phi: ",x[[3]]$phi,if(!is.null(x[[3]]$Imax))paste("\t Imax: ",round(x[[3]]$Imax,digits=2)),if(!is.null(x[[3]]$delta))if(x[[3]]$delta!=0)paste("\t delta: ",round(x[[3]]$delta,digits=2))))
cat(paste("\n\n\tBoundaries:","\t "))
